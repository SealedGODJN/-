...
public class DNAUtils {
    ...
    public boolean check(byte[] DNASeq) {
        ...
        for (int i = 0; i < DNASeq.length; i++) {
            if (DNASeq[i] == 'A') times_A++;
            else if (DNASeq[i] == 'G') times_G++;
            else if (DNASeq[i] == 'C') times_C++;
            else if (DNASeq[i] == 'T') times_T++;
            else flag = false;
        }
        return flag;
    }
    ...
    public double GC_rate() {
        return (double) (times_C + times_G) / (times_C + times_G + times_A + times_T);
    }
    ...
    public List<Integer> findSubSequenceLocation(byte[] DNASeq) {
        ...
        for (int i = 0; i < DNASeq.length; i++) {
            head = i;
            tail = head;
            while (DNASeq[tail + 1] == DNASeq[head]) {
                tail++;
            }
            if (tail - head >= 1) {
                result.add(head);
