...
public:
    bool validPath(int n, vector<vector<int>> &edges, int source, int destination)
    {
        ...
        return dfs(source, destination, adj, isVisited);
    }

    bool dfs(int source, int destination, vector<vector<int>> adj, vector<bool> isVisited) {
        ...
        for (int next : adj[source]) {
            if (!isVisited[next] && dfs(next, destination, adj, isVisited)) {
            
                return true;
                
            }
            // 这种写法会导致某些路径未被遍历，而函数走完一条路径后，直接return false
            // if (!isVisited[next]) {
                // isVisited[next] = true;
                // return dfs(next, destination, adj, isVisited);
            // }
        }
        return false;
    }
